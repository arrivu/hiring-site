...
class Quizzes::Quiz < ActiveRecord::Base
  ...
  attr_accessible :title, :description, :points_possible, :assignment_id, :shuffle_answers,
    ...
    :web_proctoring, :maximum_web_proctoring, :maximum_image_proctoring, :show_result, :percentage_of_marks,
    :allow_negative_marks, :negative_mark, :show_remaining_counts
  ...
  def set_defaults
    ...
    self.title = t('#quizzes.quiz.default_title', "Unnamed Assessment") if self.title.blank?
  ...
  def readable_type
    ...
    self.survey? ? t('#quizzes.quiz.types.survey', "Survey") : t('#quizzes.quiz.types.quiz', "Assessment")
  end
  ...
  def quiz_title
    result = self.title
    result = t('#quizzes.quiz.default_title', "Unnamed Assessment") if result == "undefined" || !result
