...
class Course < ActiveRecord::Base
  ...
  scope :by_associated_accounts, lambda{ |account_ids|
    account_ids.empty? ?
      none :
      where("EXISTS (?)", CourseAccountAssociation.where("project_account_associations.course_id=courses.id AND project_account_associations.account_id IN (?)", account_ids))
  }
  ...
  def self.valid_grade_export_types
    @valid_grade_export_types ||= {
        "instructure_csv" => {
            :name => t('grade_export_types.instructure_csv', "ArrivuApps formatted CSV"),
  ...
  def self.default_tabs
    [
        ...
        { :id => TAB_GRADES, :label => t('#tabs.grades', "Ratings"), :css_class => 'grades', :href => :course_grades_path },
