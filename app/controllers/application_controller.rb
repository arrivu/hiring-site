...
class ApplicationController < ActionController::Base
      ...
      if params[:course_id]
        ...
        if @context && session[:enrollment_uuid_course_id] == @context.id
          ...
          if session[:enrollment_uuid_count] > 4
            ...
            flash[:html_notice] = mt "#application.notices.need_to_accept_enrollment", "You'll need to [accept the enrollment invitation](%{url}) before you can fully participate in this project.", :url => course_url(@context)
          end
  ...
  def unsupported_browser
    t("#application.warnings.unsupported_browser", "Your browser does not meet the minimum requirements for Canvas. Please visit the *Canvas Guides* for a complete list of supported browsers.", :wrapper => @template.link_to('\1', 'http://guides.arrivuapps.com/s/2204/m/4214/l/41056-which-browsers-does-canvas-support'))
  end
