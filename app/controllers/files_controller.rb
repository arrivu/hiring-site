...
class FilesController < ApplicationController
  before_filter :require_user, :only => [:create_pending, :registration_image]
  before_filter :require_context, :except => [:full_index,:assessment_question_show,:registration_image,:image_thumbnail,:show_thumbnail,:preflight,:create_pending,:s3_success,:show,:api_create,:api_create_success,:api_show,:api_index,:destroy,:api_update,:api_file_status]
  ...
  def create_pending
    ...
    if authorized_action(permission_object, @current_user, permission)
      ...
      res = @attachment.ajax_upload_params(@current_pseudonym,
              named_context_url(@context, :context_files_url, :format => :text, :duplicate_handling => params[:duplicate_handling]),
              s3_success_url(@attachment.id, :uuid => @attachment.uuid, :duplicate_handling => params[:attachment][:duplicate_handling]),
              :no_redirect => params[:no_redirect],
              :upload_params => {
                'attachment[folder_id]' => params[:attachment][:folder_id] || '',
